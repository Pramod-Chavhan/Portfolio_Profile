...
export default function CustomCursor() {
  ...
  const cursorVariants = {
    ...
    clicked: { x: position.x - 16, y: position.y - 16, height: 28, width: 28 },
    hovered: { x: position.x - 24, y: position.y - 24, height: 48, width: 48 },
  }

  const cursorInnerVariants = {
    default: { x: position.x - 4, y: position.y - 4, height: 8, width: 8 },
    clicked: { x: position.x - 16, y: position.y - 16, height: 32, width: 32 },
    hovered: { x: position.x - 12, y: position.y - 12, height: 24, width: 24, opacity: 0.5 },
  }

  return (
    <div className="cursor-container pointer-events-none">
      <motion.div
